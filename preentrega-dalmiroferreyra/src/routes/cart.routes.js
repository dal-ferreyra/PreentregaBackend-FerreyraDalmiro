...
router.post("/", async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Se creó el carrito correctamente" });
    } catch (error) {
...
router.post("/:id/product/:productId", async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: "Se agregó el producto al carrito" });
    } catch (error) {
        res.status(400).json({ error: `No se pudo agregar el producto al carrito: ${error}` });
    }
});
